...
function MERN() {
    ...
    useEffect(() => {
        if (view === 'list') {
            const getPosts = async () => {
                try {
                    setLoading(true);
                    const response = await fetch('https://notes-app-uzkw.onrender.com/api/posts');
    ...
    const deletePost = async (id) => {
        ...
        try {
            const response = await fetch(`https://notes-app-uzkw.onrender.com/api/deletepost/${id}`, { method: "DELETE" });
    ...
    const onSubmitHandler = async (e, title, description, id = null) => {
        ...
        const url = id ? `https://notes-app-uzkw.onrender.com/api/updatepost/${id}` : "https://notes-app-uzkw.onrender.com/api/createpost";
    ...
    if (view === 'list') {
        return (
            <div className='h-full my-3 overflow-scroll w-full flex flex-col gap-5'>
                <h1 className='text-2xl'>Notes App</h1>
                <button onClick={() => setView('create')} className='bg-gray-700  mb-2 mx-auto text-white px-3 py-2 rounded-full'>Add Notes</button>
                {loading ? <h1 className='mt-20'>Please Wait...</h1> : posts.length > 0 ? posts.map((post) => (
                    <div key={post._id} className='border-2 border-gray-400 rounded-xl py-1 px-2 break-words'>
                ...
                )) : <h1 className='mt-20 text-xl'>No Notes</h1>}
            </div>
        );
    }
...
function PostForm({ title = "", description = "", onSubmitHandler }) {
    ...
    return (
        <form onSubmit={(e) => { setLoading(true); onSubmitHandler(e, postTitle, postDescription); }} className="flex flex-col items-start gap-9 mt-5">
            ...
            <div className='mx-auto mb-6'>
                <button type='submit' className={`px-4 py-2 ${loading ? "bg-blue-gray-50" : "bg-blue-gray-500"} ${loading ? "text-black" : "text-white"} rounded-full active:bg-blue-gray-100`} disabled={loading}>{loading ? "Wait..." : "Submit"}</button>
            </div>
        </form>
    );
}
